        ...
        >
          ...
          {models.map((m) => (
            <option key={m.id} value={m.id}>
              {m.name} ({m.provider}
              {m.customModelName ? ` · ${m.customModelName}` : ''})
            </option>
          ))}
        </select>
      )}
              ...
              {step === 0 && (
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs" style={{ color: '#848E9C' }}>
                      {zh
                        ? 'Token 格式：数字:字母数字串，如 123456789:ABCdef...'
                      ...
                      <div>
                        ...
                        >
                          <div>
                            1.{' '}
                            {zh
                              ? '在 Telegram 中搜索你刚创建的 Bot'
                              : 'Search for your newly created Bot in Telegram'}
                          </div>
                          <div>
                            2.{' '}
                            {zh ? '点击 Start 或发送' : 'Click Start or send'}{' '}
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (updated.is_bound) {
                            setStep(2)
                            toast.success(
                              zh ? '绑定成功！' : 'Bound successfully!'
                            )
                          } else {
                            toast.info(
                              zh
                                ? '尚未收到 /start，请先向 Bot 发送 /start'
                ...
                <div className="space-y-5">
                  ...
                  >
                    ...
                    {[
                      {
                        cmd: '/help',
                        desc: zh ? '查看所有命令' : 'Show all commands',
                      },
                      {
                        cmd: zh ? '查看交易员状态' : 'Show trader status',
                        desc: zh ? '自然语言查询' : 'Natural language',
                      },
                      {
                        cmd: zh ? '启动/停止交易员' : 'Start/stop trader',
                        desc: zh ? '自然语言控制' : 'Natural language control',
                      },
                      {
                        cmd: zh ? '查看持仓' : 'View positions',
                        desc: zh ? '实时持仓查询' : 'Real-time position query',
                      },
                      {
                        cmd: zh ? '配置策略' : 'Configure strategy',
                        desc: zh ? '修改交易策略' : 'Modify trading strategy',
                      },
                    ].map((item, i) => (
                      <div key={i} className="flex items-start gap-2 text-xs">
                        <code
                          className="font-mono px-1.5 py-0.5 rounded flex-shrink-0"
                          style={{ background: '#1E2329', color: '#2AABEE' }}
                        >
                          {item.cmd}
                        </code>
